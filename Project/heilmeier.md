...
Profiling the Python/NumPy software baseline (Task 5) reveals that `zscore_detect`
...
intensity of only **0.496 FLOP/byte** (Task 6).
...
regime (Task 7 roofline). The attainable performance is approximately 38.1
GFLOP/s — only 25.4% of the CPU's peak compute — because DRAM bandwidth is
...
bottleneck. The key insight from roofline analysis (Task 7) is that a 32-element
...
The required interface bandwidth is approximately 4.5 GB/s (Task 8),
