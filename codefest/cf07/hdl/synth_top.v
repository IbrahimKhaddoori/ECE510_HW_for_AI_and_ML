...
module compute_core (
    ...
    localparam S_IDLE=3'd0, S_LOAD=3'd1, S_MAC=3'd2, S_BIAS=3'd3, S_MSE=3'd4, S_DONE=3'd5;
    ...
    `define BIAS_LANE(ACC, BA, RES) \
        bv = (ACC >>> 7) + {{24{weight_mem[BA][7]}}, weight_mem[BA]}; \
    ...
    // MSE diff
    wire signed [8:0] mse_diff = $signed({1'b0, orig_in[mse_idx]}) - $signed({1'b0, act_out[mse_idx]});
